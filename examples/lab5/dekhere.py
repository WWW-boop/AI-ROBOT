...
#     template_paths = [
#         "./Lab5_p_fern/template_picture/very_small.jpg",
#         "./Lab5_p_fern/template_picture/small.jpg",
#         "./Lab5_p_fern/template_picture/medium.jpg",
#     ]
...
#     image, templates, stride=50, padding=10, scales=[1, 1.25]
# ):
#     max_similarity = 0
...
#             template_scale.append(scaled_template)

#     for template in template_scale:
#         max_similarity = 0
#         h, w = template.shape[:2]
#         print(f"-----{template_scale.index(template)}----")
...
#                 similarity = cosine_similarity(template, window)
#                 print(f"{template_scale.index(template)} {similarity}")
#                 if similarity > max_similarity:
#                     max_similarity = similarity
#                     score_template[f"key_{template_scale.index(template)}"] = [
#                         max_similarity,
#                         w,
#                         h,
#                         x - padding,
...
#     final = max(score_template.items(), key=lambda x: x[1][0])
#     # position, size, similarity
#     return [final[1][3], final[1][4]], [final[1][1], final[1][2]], final[1][0]
...
def sliding_window_cosine_similarity(
    image, templates, stride=100, padding=10, scales=[0.75, 1, 1.25, 1.5]
):
    max_similarity = 0
    best_position = (0, 0)
    best_size = (0, 0)
        ...
        if similarity > 0.9:  # Adjust this threshold as needed
            ...
            if count >= 1:
                ...
                speed_x = (
                    p
                    * err_x
                    # + d * ((err_x - prev_err_x) / (current_time - prev_time))
                    # + i * (accumulate_err_x) * (current_time - prev_time)
                )
                speed_y = (
                    p
                    * err_y
                    # + d * ((err_y - prev_err_y) / (current_time - prev_time))
                    # + i * (accumulate_err_y) * (current_time - prev_time)
                )
