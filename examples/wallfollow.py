...
def front_wall():
    ...
    if sensor_data['ad_value'] is not None:
        return sensor_data['ad_value'][0] < 100  # Assuming front sensor is at index 0
    return False
...
def wall_following_solve():
    while True:
        if front_wall():
            ...
            if left_wall(): # ถ้าทางเดินข้างหน้ามีสิ่งกีดขวาง ให้เลี้ยวซ้าย
                 move_forward()
            else:
                turn_left()  # ถ้ายังไม่ว่าง ให้เลี้ยวซ้ายอีกครั้งเพื่อหลีกเลี่ยงสิ่งกีดขวางใหญ่
