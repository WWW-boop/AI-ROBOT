...
def front_wall():
    if tof_distance is not None:
        return tof_distance < 230  # ถ้าต่ำกว่า 230 คือเจอกำแพง
    return False
...
def wall_following_solve():
    ...
    while True:
        time.sleep(0.1)
        print(f"right_wall: {right_wall()}, left_wall: {left_wall()}, front_wall: {front_wall()}, stick_right_wall: {stick_right_wall()}")
        if right_wall():  # ถ้าเจอกำแพงด้านขวา
        ...
        elif not right_wall():  # ถ้าไม่พบกำแพงขวา
            for i in range(3):
                move_forward()
            ...
            if not right_wall():
                ...
                for i in range(7):
                    move_forward()
        ep_gimbal.recenter().wait_for_completed()  # ขยับ gimbal มาตรงกลางทุกครั้งที่ทำตามคำสั่งข้างต้น
