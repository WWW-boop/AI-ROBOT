...
def turn_left():
    ...
    ep_gimbal.recenter(pitch_speed=200, yaw_speed=200).wait_for_completed()
    update_direction('left')  # Pass the 'left' turn argument
    print("------- turn left -------")

def turn_right():
    ep_robot.chassis.move(x=0, y=0, z=-91, z_speed=100).wait_for_completed()
    ep_gimbal.recenter(pitch_speed=200, yaw_speed=200).wait_for_completed()
    update_direction('right')  # Pass the 'right' turn argument
    print("------- turn right -------")

def turn_around():
    ep_robot.chassis.move(x=0, y=0, z=180, z_speed=100).wait_for_completed()
    ep_gimbal.recenter(pitch_speed=200, yaw_speed=200).wait_for_completed()
    update_direction('back')  # Pass the 'back' turn argument
    print("------- turn around -------")
...
def update_direction(turn=None):
    ...
    # No need to update the direction when moving forward
