...
# Add global variable to store positions where the blaster was fired
fired_positions = []
chick_position = []
def update_plot(frame):
    ...
    if fired_positions:
        ...
        print(f"Plotting fired positions: {fired_positions}")  # Debugging line
        plt.draw()  # Ensure the plot is redrawn
        plt.pause(0.01)
    return robot_plot,
...
def detect():
    global now_pos, detection_duration, detection_start_time, fired_positions
    print('detect')
    time.sleep(1)  
    frame_count = 0
    ...
    while True:
        ...
        if bottle_bbox:
            ...
            if detection_duration > 3:
                ...
                if direction_facing == 'N':
                    fired_positions.append([now_pos[0]+1, now_pos[1]])
                elif direction_facing == 'W':
                    fired_positions.append([now_pos[0], now_pos[1]-1])
                elif direction_facing == 'E':
                    fired_positions.append([now_pos[0], now_pos[1]+1])
                elif direction_facing == 'S':
                    fired_positions.append([now_pos[0]-1, now_pos[1]])
