...
blaster_fired_positions = []
robot_position, = ax.plot([], [], 'ro', label="Robot Position")
blaster_position_marker, = ax.plot([], [], 'bo', label="Blaster Fired", marker='x', markersize=10)
...
def animate(i):
    ...
    robot_position.set_data(now_pos[0], now_pos[1])

    # If blaster was fired, update the position
    blaster_position_marker.set_data([pos[0] for pos in blaster_fired_positions], 
                                     [pos[1] for pos in blaster_fired_positions])

    return robot_position, blaster_position_marker
...
if __name__ == '__main__':
    ani = FuncAnimation(fig, animate, frames=100, interval=500, blit=True)
    plt.legend()
    plt.show()
