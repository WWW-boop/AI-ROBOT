...
def main():
    ...
    # Timer for detecting and confirming the object is centered
    ...
    while True:
        ...
        if bottle_bbox:
            ...
            # If the error is small, start the timer
            if abs(err_x) < 10 and abs(err_y) < 10:
                if start_detection_time is None:
                    start_detection_time = time.time()
                elif time.time() - start_detection_time >= 1:  # If 1 second has passed, capture the image
                    cv2.imwrite("igametum.png", img)
                    print("Screenshot captured and saved as 'detected_object.png'.")
                    start_detection_time = None  # Reset the timer after capturing
            else:
                start_detection_time = None  # Reset the timer if the target is not centered
            ...
            # Reset detection timer if no target is detected
            start_detection_time = None
